...
- 通过更广的隐式 Skill 触发和可选 Codex 生命周期提醒，覆盖仓库分析、调试、审查、
  实现、测试与发布；持久写入仍必须通过证据门禁，不会自动发生。
...
`2.0.3` 已正式发布，对应 `v2.0.3` tag。稳定版本请使用下面带 tag 的安装命令；
...
`UserPromptSubmit` 与 `Stop` 命令，并在需要生命周期提醒时信任它们。`2.0.3` 更新了 Windows
Hook 命令，升级用户需要重新检查并信任这两个命令；Windows 生命周期提醒不再依赖用户 PATH
中的 Python，也不再解析 Hook stdin JSON。即使 Hook 未信任或已禁用，更广的 Skill metadata 与手工 `$tracebook` 调用仍可使用。
...
- 不需要也不提供 API key、cloud sync、MCP server、vector database 或后台 daemon。
  Codex 插件会附带只负责提醒、不会写知识的生命周期 Hook；Hook 需要用户信任，也可禁用，
  不影响确定性 runner 或手工调用 Skill。
...
- `2.0.3` 使用 schema-v2 authority 页面。旧版本知识根不会被迁移、导入或与新格式混写；
  使用 v2 时请将 `TRACEBOOK_ROOT` 指向新的空知识根。

- 不迁移、不发现、也不自动导入现有知识根目录。
- 不提供 cloud sync、MCP server、vector database、daemon 或后台服务。
- 生命周期 Hook 不会自动写知识，只注入读取和最终门禁提醒。
