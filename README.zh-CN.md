...
知识根目录与业务代码相互分离。因此，多个仓库可以共用一个本地知识系统，无需向这些
业务仓库安装文件、hook 或服务。
...
构建集成、诊断失败或需要可复现命令时使用 runner。自然语言 Plugin 用法仍是主要入口。
下面的示例假定 shell 位于业务仓库根目录，`SKILL_DIR` 指向已安装的 Tracebook Skill，
并且已按前文设置 `TRACEBOOK_ROOT`。
...
生成的链接使用标准 Markdown 标签加相对路径语法。为兼容手工编辑的 Obsidian 知识，
健康检查可以同时审计 Markdown 链接和 Wikilink，但 Tracebook 不会生成 Wikilink。
受治理的存储位置详见
