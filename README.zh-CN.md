...
`2.0.2` 已正式发布，对应 `v2.0.2` tag。稳定版本请使用下面带 tag 的安装命令；
...
`UserPromptSubmit` 与 `Stop` 命令，并在需要生命周期提醒时信任它们。`2.0.2` 保留了 Windows
...
- `2.0.2` 使用 schema-v2 authority 页面。旧版本知识根不会被迁移、导入或与新格式混写；
