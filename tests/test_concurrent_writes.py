    ...
    def test_opposite_lifecycle_captures_keep_one_entity_and_one_pointer(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            try:
                ...
                while len(list(ready.iterdir())) != 2:
                    if any(process.poll() is not None for process in processes):
                        failure_reason = "a lifecycle child exited before the barrier"
                        break
                    if time.monotonic() >= ready_deadline:
                        failure_reason = "lifecycle children timed out before the barrier"
                        break
                    time.sleep(0.02)
                if failure_reason is None:
                    gate.touch()
                    for index, process in enumerate(processes):
                        try:
                            ...
                            failure_reason = f"lifecycle child {index} timed out"
                            break
                        results[index] = (process.returncode, stdout, stderr)
            finally:
                collected = self._cleanup_and_collect(processes, results)
