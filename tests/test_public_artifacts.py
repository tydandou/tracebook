...
class PublicArtifactsTest(unittest.TestCase):
    ...
    def test_guides_document_root_provenance_and_structured_health_results(self) -> None:
        english = (ROOT / "README.md").read_text(encoding="utf-8")
        chinese = (ROOT / "README.zh-CN.md").read_text(encoding="utf-8")

        for token in ("root_source", "root_initialized", "root_warning", "findings"):
            self.assertIn(token, english)
            self.assertIn(token, chinese)
        self.assertIn("snapshot-prune", english)
        self.assertIn("快照剪枝", chinese)
    ...
    def test_claude_plugin_manifest_and_marketplace_expose_tracebook(self) -> None:
        ...
        self.assertEqual("4.0.7", marketplace["plugins"][0]["version"])
    ...
    def test_release_docs_explain_v1_workflow_and_boundaries(self) -> None:
        ...
        self.assertIn("## [4.0.0] - 2026-07-27", changelog)
        self.assertIn("## [3.3.2] - 2026-07-24", changelog)
        self.assertIn("## [3.3.0] - 2026-07-23", changelog)
    ...
    def test_guides_document_read_only_transaction_diagnostics(self) -> None:
        ...
        self.assertIn("TRANSACTION_RECOVERY_REQUIRED", english)
        self.assertIn("`transactions` is read-only", english)
        self.assertIn("recover-transactions", chinese)
        self.assertIn("TRANSACTION_RECOVERY_REQUIRED", chinese)
    ...
    def test_ci_verifies_supported_python_versions_on_linux_and_windows(self) -> None:
        ...
        for required in (
            ...
            'PowerShell transport engine: $($PSVersionTable.PSVersion)',
        ):
    ...
    def test_public_versions_and_powershell_evidence_are_current(self) -> None:
        ...
        chinese_site = (ROOT / "site" / "zh" / "index.html").read_text(encoding="utf-8")
        llms = (ROOT / "site" / "llms.txt").read_text(encoding="utf-8")

        for artifact in (site, chinese_site, llms):
            self.assertIn("v4.0.7", artifact)
            self.assertNotIn("v4.0.3", artifact)
        self.assertIn('"softwareVersion": "4.0.7"', site)
    ...
    def test_bilingual_guides_describe_the_release_and_complete_deep_scope(self) -> None:
        ...
        self.assertIn("The `4.0.7` release is available", english)
        ...
        self.assertIn("`4.0.7` 已发布", chinese)
