...
class SystemRelationCandidateTest(unittest.TestCase):
    ...
    def _fixture(self, base: Path) -> tuple[Path, Path, Path, dict]:
        """A nested layout: `sample_service` is registered inside the `docs` working tree.
    ...
    def test_no_candidate_once_a_system_relation_exists(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            self._run(
                ...
                "--target-project-id", ids["sample_service"], "--kind", "designs",
            )
    ...
    def test_shared_topic_words_alone_never_flag(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            self._capture(root, sample_service, "order-refund-impl", ["src/sample_service.go:L1"])

            self.assertEqual([], self._candidates(root, docs))
            self.assertEqual([], self._candidates(root, sample_service))
    ...
    def test_nested_project_citing_its_own_source_never_flags_its_parent(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            base = Path(temp)
            root, docs, sample_service, _ = self._fixture(base)
            # sample_service lives inside the docs working tree, so its own files are also
            # under docs' registered location.
            self._capture(root, sample_service, "owns-its-source", ["src/sample_service.go:L1"])
